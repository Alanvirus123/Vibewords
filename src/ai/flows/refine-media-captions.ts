...
const RefineMediaCaptionsOutputSchema = z.object({
  refinedCaptions: z
    .array(z.string())
    .describe('The refined set of 4 caption suggestions.'),
});
...
const refineMediaCaptionsPrompt = ai.definePrompt({
  ...
  Your goal is to refine the initial captions based on the user feedback to create a new set of four improved caption suggestions.
  ...
  Return an array of 4 refined captions.`,
});
